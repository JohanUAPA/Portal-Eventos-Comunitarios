...
if (isset($_GET['message'])) {
    if ($_GET['message'] == 'converted') {
        $message = 'El administrador ha sido convertido en usuario.';
...
// Consultar usuarios con rol de "admin"
$sql = "SELECT * FROM usuarios WHERE rol = 'admin'";
$result = $conn->query($sql);

if ($result->num_rows == 0) {
    die("No se encontraron administradores.");
}
?>
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="text-center mb-4">Administrar Administradores</h2>
            ...
            <tbody>
                <?php while ($admin = $result->fetch_assoc()) { ?>
                    <tr>
                        ...
                        <td>
                            <!-- Formulario para convertir en usuario -->
                            <form action="convert_to_user.php" method="POST" style="display:inline-block;">
                                <input type="hidden" name="id" value="<?php echo $admin['id']; ?>">
                                <button type="submit" class="btn btn-warning btn-sm">Convertir en Usuario</button>
                            </form>
